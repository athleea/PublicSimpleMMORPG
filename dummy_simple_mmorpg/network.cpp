...
const static int MAX_PACKET_SIZE = 4096;
const static int MAX_BUFF_SIZE = 4096;
...
void ProcessPacket(int ci, unsigned char packet[])
{
	switch (packet[2]) {
		...
		case SC_HIT: break;
		case SC_OBTAIN: break;
		default: 
			std::cout << "[Error] recvied default packet\n";
	}
}
...
void Worker_Thread()
{
	while (true) {
		...
		if (OP_RECV == over->event_type) {
			...
			while (io_size > 1) {
				if (0 == psize) psize = buf[0] + buf[1] * 256;
